'''Code for extracting json file from html file without selecting case type from drop down menu '''
...
while True:
    try:
        ...
        state_name = input("State Name (e.g., Bombay, Delhi): ")  # Added state name input
        ...
        # url = f"https://hcservices.ecourts.gov.in/ecourtindiaHC/cases/case_no.php?state_cd={state_code}&dist_cd={dist_code}&court_code={court_code}&stateNm={state_name}"
        # data = s.get(url)
        ...
        # csrf_elements = soup.find_all('input', attrs={'name': '__csrf_magic'})
        # print(csrf_elements)
        # if not csrf_elements:
        #     print("CSRF token not found! Check the response HTML for changes.")
        # else:
